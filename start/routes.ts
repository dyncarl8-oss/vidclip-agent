...
router.get('*', async ({ response, request }) => {
  if (request.url().startsWith('/api') || request.url().startsWith('/v2')) {
    return response.status(404).json({ message: 'API Route not found' })
  }
  const appPath = path.join(process.cwd(), 'public', 'index.html')
  if (fs.existsSync(appPath)) {
    return response.download(appPath)
  }
  return response.status(404).send('Frontend not built. Run pnpm build to generate assets.')
})
